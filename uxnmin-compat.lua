-- version of uxnmin.lua made to work on more Lua versions
...
-- }
if bit32 ~= nil then bit = bit32 end
local band=bit.band
local bor=bit.bor
local bxor=bit.bxor
...
local function emu_dei(port)
    return dev[port%0x100]
end

local function emu_deo(port, value)
    dev[port%0x100] = value
    if port == 0x11 then
        console_vector = (dev[0x10] * 0x100) + value
...
local function DEC(m) local t=(ptr[m]-1)%0x100;ptr[m]=t;return t end
local function INC(m) local t=ptr[m];ptr[m]=(t+1)%0x100;return t end
local function IMM() a[1]=(ram[pc] * 0x100) + ram[(pc+1)%0x10000];pc = (pc+2)%0x10000 end
local function MOV() pc = d > 0 and a[1] or (pc + (a[1]+0x80)%0x100-0x80) % 0x10000 end
local function POx(o,m) o[1] = stk[r][DEC(r)]; if m > 0 then o[1] = o[1] + (stk[r][DEC(r)] * 0x100) end end
local function PUx(i,m,s) if m > 0 then c = i; stk[s][INC(s)] = rshift(c, 8) % 0x100; stk[s][INC(s)] = c % 0x100 else stk[s][INC(s)] = i%0x100 end end
...
local function POK(o,v,m) ram[o] = v[0]; if d > 0 then ram[band(o + 1, m)] = v[1] end end
local function PEK(i,v,m) v[0] = ram[i]; if d > 0 then v[1] = ram[band(i + 1, m)] end; PUT(v,r) end
ops = {
    [0x00]=function() return 1 end, -- BRK
    [0x20]=function() if stk[0][DEC(0)] > 0 then IMM(); pc=(pc+a[1])%0x10000 else pc=(pc+2)%0x10000 end end, -- JCI
    [0x40]=function() IMM(); pc=(pc+a[1])%0x10000 end, -- JMI
    [0x60]=function() IMM(); PUx(pc, 1, 1); pc=(pc+a[1])%0x10000 end, -- JSI
    [0x80]=function() stk[0][INC(0)] = ram[pc]; pc=(pc+1)%0x10000 end, -- LIT
    [0xa0]=function() stk[0][INC(0)] = ram[pc]; stk[0][INC(0)] = ram[(pc+1)%0x10000]; pc=(pc+2)%0x10000 end, -- LI2
    [0xc0]=function() stk[1][INC(1)] = ram[pc]; pc=(pc+1)%0x10000 end, -- LIr
    [0xe0]=function() stk[1][INC(1)] = ram[pc]; stk[1][INC(1)] = ram[(pc+1)%0x10000]; pc=(pc+2)%0x10000 end, -- L2r
}
OPC(0x01, function() POx(a,d) end, function() PUx(a[1] + 1,d, r) end) -- INC
OPC(0x02, function() ptr[r]=(ptr[r]-1-d)%0x100 end, function()end) -- POP
OPC(0x03, function() GOT(x) ptr[r]=(ptr[r]-1-d)%0x100 end, function() PUT(x,r) end) -- NIP
...
OPC(0x0e, function() POx(a,d) end, function() PUx(pc,1,1-r); MOV() end) -- JSR
OPC(0x0f, function() GOT(x) end, function() PUT(x,1-r) end) -- STH
...
OPC(0x12, function() POx(a,0) end, function() PEK(pc + (a[1]+0x80)%0x100-0x80, x, 0xffff) end) -- LDR
OPC(0x13, function() POx(a,0); GOT(y) end, function() POK(pc + (a[1]+0x80)%0x100-0x80, y, 0xffff) end) -- STR
...
OPC(0x1b, function() POx(a,d); POx(b,d) end, function() if a[1] > 0 then a[1] = floor(b[1] / a[1]) end PUx(a[1], d,r) end) -- DIV
OPC(0x1c, function() POx(a,d); POx(b,d) end, function() PUx(band(b[1], a[1]), d,r) end) -- AND
OPC(0x1d, function() POx(a,d); POx(b,d) end, function() PUx(bor(b[1], a[1]), d,r) end) -- ORA
OPC(0x1e, function() POx(a,d); POx(b,d) end, function() PUx(bxor(b[1], a[1]), d,r) end) -- EOR
OPC(0x1f, function() POx(a,0) POx(b,d) end, function() PUx(lshift(rshift(b[1], (a[1] % 0x10)), rshift(a[1], 4)), d,r) end) -- SFT
